...
import chartjstest.util.Opener;
import chartjstest.util.TestFactory;
...
public class CeauChartTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            // using the library's test examples
            //PieTest jsPieTest = new PieTest();
            //Chart jsPieChart = jsPieTest.createChart();

            PieChart reportPieChart = new PieChart();
            PieData reportPieData = new PieData();
            reportPieData.addLabel(sites[0]);
            PieDataset reportPieDataset = new PieDataset();
            reportPieDataset.addData(4);
            reportPieDataset.addData(30);
            reportPieDataset.addData(372);
            reportPieDataset.addData(90);
            reportPieChart.setData(reportPieData);
            //reportPieDataset.setData(new ArrayList<>(Arrays.asList(4,30,372,90))); // haven't got the syntax quite right for this yet
                    
            reportPieChart.setData(TestFactory.randomInstance(PieData.class));
            reportPieChart.setOptions(TestFactory.randomInstance(PieOptions.class));
            
            //System.out.println("Chart JSON = " + reportPieChart.toJson());
         
            Opener.inBrowser(reportPieChart);
        } catch (Exception e) {
            e.printStackTrace();
        }
